...
# Created by Hugues NOMENJANAHARY
#################################################################
FROM ubuntu:18.04
MAINTAINER Hugues NOMENJANAHARY <huguesnmj@gmail.com>

ARG DEBIAN_FRONTEND=noninteractive

## Add Key
RUN gpg --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys 9BE6ED79
...
RUN echo "deb http://us.archive.ubuntu.com/ubuntu/ bionic-updates main restricted universe multiverse" >> /etc/apt/sources.list
RUN echo "deb http://us.archive.ubuntu.com/ubuntu/ bionic-security main restricted universe multiverse" >> /etc/apt/sources.list
...
# Configure Timezone
RUN echo "tzdata tzdata/Areas select Africa\ntzdata tzdata/Zones/Africa select Nairobi" > /tmp/tz ; debconf-set-selections /tmp/tz; rm /etc/localtime /etc/timezone; dpkg-reconfigure -f noninteractive tzdata
...
RUN curl -k https://raw.githubusercontent.com/HuguesNMJ/zimbra-docker/main/dsn-auto.sh > /srv/dns-auto.sh
RUN chmod +x /srv/dns-auto.sh

# Copy rsyslog services
RUN mv /etc/init.d/rsyslog /tmp/
RUN curl -k https://raw.githubusercontent.com/HuguesNMJ/zimbra-docker/main/rsyslog > /etc/init.d/rsyslog
